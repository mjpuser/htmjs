...
SpatialPooler.prototype = {
	...
	_inhibit: function(overlaps) {
		...
		for (var i = 0; i < overlaps.length; i++) {
			...
			//console.log('bucket', offset * range, 'range', range, 'inhRadius', radius, 'id', overlaps[i].id, 'inhibitors', inhibitors);
			if (overlaps[i].id > offset * range) {
				inhibitors = inhibitors.concat(rangeWinners.slice(0, this.desiredLocalActivity).toArray());
				offset++;
				i--;
