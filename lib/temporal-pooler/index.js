...
var TemporalPooler = function(options) {
	...
	this.dendritesPerSegment = options.dendritesPerSegment || 40;
	Segment.ACTIVATION_THRESHOLD = 3; //Math.floor(3 / 4 * this.dendritesPerSegment);
	Segment.LEARNING_THRESHOLD = 3; // 
	Segment.MIN_THRESHOLD = 0; // remove segment if falls below this.
...
TemporalPooler.prototype = {
	...
	teachSynapses: function(cell) {
		var previousStep = time.getPreviousStep();
		var currentStep = time.getCurrentStep();

		cell.segments.forEach(function(segment) {
			segment.synapses.forEach(function(synapse) {
				synapse.reinforce(previousStep, currentStep);
				if (!previousStep.activeSynapses.has(synapse)) {
					var cell = previousStep.activeCellList[Math.floor(previousStep.activeCellList.length * Math.random())];
					if (cell) {
						synapse.cell = cell;
						cell.synapses.push(synapse);
					}
				}
			});
		});
	...
	predictAndLearnPhase2: function() {
		...
		currentStep.activeSegments.forEach(function(segment) {
			segment.synapses.forEach(function(synapse) {
				var isActive = currentStep.activeCells.has(synapse.cell);
				if (isActive) {
