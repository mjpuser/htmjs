...
var TemporalPooler = function(options) {
	...
	Segment.ACTIVATION_THRESHOLD = 6; //Math.floor(3 / 4 * this.dendritesPerSegment);
	Segment.LEARNING_THRESHOLD = 3; // 
	Segment.MIN_THRESHOLD = 1; // remove segment if falls below this.
...
TemporalPooler.prototype = {
	...
	predict: function() {
		...
		this.input.forEach(function(id) {
			...
			column.cells.forEach(function(cell) {
				...
				if (previousStep.predictiveCells.has(cell)) {
					segment = cell.getActiveSegment(previousStep);
					if (segment.sequence) {
						bottomUpPredicted = true;
						currentStep.activeCells.add(cell);
					}
				}
			});
		...
		currentStep.activeSegments.forEach(function(segment) {
	...
	predictAndLearn: function() {
		...
		this.predictAndLearnPhase3(segmentUpdateList);
	...
	predictAndLearnPhase1: function() {
		this.input.forEach(function(id) {
			...
			if (!chosen) {
				...
				previousStep.sequenceSegment.add(bestMatchingSegment);
				if (!segmentUpdateMap[bestMatchingSegment.cell.id]) {
					segmentUpdateMap[bestMatchingSegment.cell.id] = [];
				}
				segmentUpdateMap[bestMatchingSegment.cell.id].push(segmentUpdate);
			}
		}, this);
	},
	predictAndLearnPhase2: function(segmentUpdateMap) {
		time.activeSegments.forEach(function(segment) {
			...
			if (!segmentUpdateMap[segment.cell.id]) {
				segmentUpdateMap[segment.cell.id] = [];
			}
			segmentUpdateMap[segment.cell.id].push(activeUpdate);
			if (!segmentUpdateMap[predictiveSegment.cell.id]) {
				segmentUpdateMap[predictiveSegment.cell.id] = [];
			}
			segmentUpdateMap[predictiveSegment.cell.id].push(predictiveUpdate);
		}, this);
	},
	predictAndLearnPhase3: function(segmentUpdateMap) {
		var currentStep = time.getCurrentStep();
		time.getCurrentStep().learningCells.forEach(function(cell) {
			this.adaptSegments(segmentUpdateMap[cell.id], true);
