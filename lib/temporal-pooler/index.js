...
TemporalPooler.prototype = {
		...
		this.input.forEach(function(id) {
			...
			column.cells.forEach(function(cell) {
				if (previousStep.predictiveCells.has(cell)) {
					...
					cell.markActive();
					cellsPerColumnRight++;
					cellsPerColumnRightList.push(cell.id % 4);
				}
			});
			//cellsPerColumnRightList.push(cellsPerColumnRight);
					...
					if (!synapse.isConnected()) {
						...
						if (segments.length) {
							...
							if (bestMatchingSegment) {
								//console.log('best matching', bestMatchingSegment.cell.id % 4);
	...
	getBestMatchingSegment: function(segments, step) {
		...
		segments.forEach(function(segment) {
			...
			if (
				((theseImpulses >= bestImpulses && segment.learning === bestMatchingSegment.learning) ||
	...
	getActivatedColumnSegments: function(step) {
		var cell = step.activeCellList[Math.floor(Math.random() * step.activeCellList.length)];
