...
var TemporalPooler = function(options) {
	...
	assert.ok(is.number(this.columnLength) && this.columnLength > 0, 'columns must be a number > 0');
	...
	this.maxSegmentsPerCell = options.maxSegmentsPerCell || 6;
	this.maxDendritesPerSegment = options.maxDendritesPerSegment || 40; // used for learning.  A segment to grow up to this number of dendrites

	Segment.ACTIVATION_THRESHOLD = options.activationThreshold || 10; //Math.floor(3 / 4 * this.dendritesPerSegment);
	Segment.MIN_THRESHOLD = options.minThreshold || 3; // remove segment if falls below this.

	this.dendriteSpan = options.dendriteSpan || 30;
	this.permanenceDec = options.permanenceDec || 0.03;
...
TemporalPooler.prototype = {
	...
	predict: function() {
		...
		return this.getPredictedSdr();
	},
	getPredictedSdr: function() {
