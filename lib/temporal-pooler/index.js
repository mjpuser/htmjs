...
TemporalPooler.prototype = {
		...
		for(var i = 0; i < this.numColumns; i++) {
			...
			for (var k = 0; k < this.cellsPerColumn; k++) {
				cell = new Cell({
					...
					onLearn: function() {
						self.learnCells[time.getTick()].add(this);
					}
				});
				column.addCell(cell);
	...
	predictAndLearn: function() {
		var activeColumns;
		var inference = [];
		var predictedCells = this.predictedCells[time.getNextTick()];
		var cells = this.predictedCells[time.getPreviousTick()];
		if (cells) {
			cells.forEach(function(cell) {
				cell.markInactive(time.getPreviousTick());
			});
		}

		// reset old stuff
		this.activeColumns[time.getPreviousTick()] = new Set();
		this.predictedCells[this.getPreviousTick()] = new Set();
		this.activeSegments[this.getNextTick()] = new Set();
		...
		this.activeSegments[time.getPreviousTick()].forEach(function(segment) {
			segment.markInactive(time.getPreviousTick());
		});
